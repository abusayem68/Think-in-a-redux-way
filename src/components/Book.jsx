...
const Book = ({ book }) => {
  const { id, name, author, price, thumbnail, featured, rating } = book || {};
  // render rating
  let stars;
  if (rating <= 0 && rating > 5) {
    const ratingsArray = Array.from({ length: 5 }, (v, i) => i + 1);
    stars = ratingsArray.map((rating, i) => (
      <svg
        key={i}
        viewBox="0 0 20 20"
        fill="noColor"
  ...
  if (rating >= 0 && rating <= 5) {
    const ratingsArray = Array.from({ length: rating }, (v, i) => i + 1);
    stars = ratingsArray.map((rating, i) => (
      <svg
        key={i}
        viewBox="0 0 20 20"
